		...
		<div className={contactPageStyles.pageContainer}>
			<div className={contactPageStyles.gridContainer}>
				<div className={contactPageStyles.formContainer}>
					...
					<p className={contactPageStyles.formSubtitle}>We'd love to hear from you. Send a message or request a service.</p>

					<form onSubmit={handleSubmit} className={contactPageStyles.formSpace}>
						<div className={contactPageStyles.formGrid}>
							<div>
								<label className={contactPageStyles.label}>Full Name</label>
								<input
									name="name"
									value={form.name}
						...
						<div className={contactPageStyles.formGrid}>
							...
							<div>
								<label className={contactPageStyles.label}>Preferred Date (optional)</label>
								<input name="date" value={form.date || ""} onChange={handleChange} placeholder="YYYY-MM-DD" className={contactPageStyles.input} />
							</div>
						</div>

						<div>
							<label className={contactPageStyles.label}>Message</label>
							<textarea name="message" value={form.message} onChange={handleChange} rows={4} className={contactPageStyles.textarea} placeholder="Tell us briefly why you're contacting us..." />
							{errors.message && <p className={contactPageStyles.error}>{errors.message}</p>}
						</div>

						<div className={contactPageStyles.buttonContainer}>
							<button type="submit" className={contactPageStyles.button}>Send via WhatsApp</button>
							{sent && <div className={contactPageStyles.sentMessage}>Message prepared — opening WhatsApp</div>}
						</div>
					</form>
				</div>
				...
				<div className={contactPageStyles.infoContainer}>
					<div className={contactPageStyles.infoCard}>
						...
						<div className={contactPageStyles.infoItem}><Phone /> +91 82994 31275</div>
						<div className={contactPageStyles.infoItem}><Mail /> contact@hospital.example</div>
						<div className={contactPageStyles.infoItem}><MapPin /> Gomti Nagar, Lucknow, Uttar Pradesh</div>
					</div>

					<div className={contactPageStyles.infoCard}>
						<h3 className={contactPageStyles.infoTitle}>Opening Hours</h3>
						<div className={contactPageStyles.hoursContainer}>
							<div className={contactPageStyles.hoursTitle}>Mon - Sat</div>
							<div className={contactPageStyles.hoursText}>08:00 AM - 08:00 PM</div>
							<div className={contactPageStyles.hoursTitle}>Sunday</div>
							<div className={contactPageStyles.hoursText}>09:00 AM - 02:00 PM</div>
						</div>
					</div>
