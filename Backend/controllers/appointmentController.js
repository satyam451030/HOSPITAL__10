...
export const getAppointmentsByPatient = async (req, res) => {
    try{
        ...
        if (!resolvedCreatedBy && !req.query.mobile) {
            return res.status(401).json({success: false, message: "Missing createdBy parameter or user authentication." });
        }
...
export const createAppointment = async (req, res) => {
    ...
    const clerkUserId = resolveClerkUserId(req);
        if(!clerkUserId && !ownerFromBody){
            return res.status(401).json({ success: false, message: "User authentication required." });
        }
        if(!doctorId || !patientName || !mobile || !date || !time || (!fee && fee !== 0 && !fees)){
        ...
        // Duplicate check: Ensure no existing appointment with same doctor, date, and time
        const existingAppointment = await Appointment.findOne({ doctorId, 
            ...
            try {
                ...
                console.warn("Doctor not found for ID:", e?.message || e);
            }
if(!doctor)
...
//  to confirm the online payment and update the appointment status
export const confirmPayment = async (req, res) => {
    try {
const { session_id } = req.query;
if (!session_id) {
  return res.status(400).json({ success: false, message: "Missing session_id parameter" });
}
if(!stripe){
...
export const updateAppointment = async (req, res) => {
  try {
    ...
    const updated = await Appointment.findByIdAndUpdate(id, update, 
      { new: true,runValidators: true }).populate({path: "doctorId", select: "name speciality image"}).lean();
    return res.json({ 
      success: true, 
      appointment: updated });
  }
...
export const getStats = async (req, res) => {
    ...
    const recent = await Appointment.countDocuments({ doctorId: req.params.doctorId, createdAt: { $gte: sevenDaysAgo } });
...
export const getAppointmentsByDoctor = async (req, res) => {
    try {
        ...
        if(!doctorId){
            return res.status(400).json({ success: false, message: "Missing doctorId parameter" });
        }
