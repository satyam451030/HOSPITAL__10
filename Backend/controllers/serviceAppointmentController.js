...
try{
  const { serviceId, mobile, status, page: pageRaw = 1, limit: limitRaw = 50, search = "" } = req.query;
	...
	return res.json({
		...
		appointments,
		meta: {
			page,limit, total, count: appointments.length
		}
	})
}
catch(error){
console.error("getServiceAppointments error:" ,error);
	...
	// to get Service AppointedmentById
...
if(!appt) return res.status(404).json({
	success: false,
	message: "Not found the appointments"
});
...
const appt = await ServiceAppointment.findById(id);
 ...
 if (!appt) return res.status(404).json({ success: false, message: "Not found" });
    ...
    if (appt.payment && appt.payment.status !== "Paid") appt.payment.status = "Pending";
await appt.save();
...
console.error("getServiceAppointmentStats :" ,error);
	return res.status(500).json({
		success:false,
		message: "Server Error"
	})
...
console.error("getServiceAppointmentsByPatient :" ,error);
	return res.status(500).json({
		success:false,
		message: "Server Error"
	})
