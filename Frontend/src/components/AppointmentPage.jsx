...
const AppointmentPage = () => {
	...
	const loadDoctorAppointments = useCallback(async () => {
		...
		try {
			...
			if (!res.ok) throw new Error((json && json.message) || `Status ${res.status}`);

			const fetched = json?.appointments ?? json?.data ?? json ?? [];
			...
			setError((prev) => (prev ? prev + " | Doctors failed" : "Failed to load doctor appointments."));
			setDoctorAppts([]);
		} finally {
	...
	const loadServiceAppointments = useCallback(async () => {
		...
		try {
			const res = await fetch(`${API_BASE}/api/service-appointments/me`, { headers });
			const json = await res.json().catch(() => null);
			if (!res.ok) throw new Error((json && json.message) || `Status ${res.status}`);
			...
			console.error("Error calling /api/service-appointments/me:", err?.message || err);
			setError((prev) => (prev ? prev + " | Services failed" : "Failed to load service appointments."));
			setServiceAppts([]);
		} finally {
	...
	const serviceData = useMemo(() => {
		return serviceAppts
			.map((s) => {
				...
				const image =
					svc.imageUrl || svc.image || svc.imageSmall || s.serviceImage?.url || s.serviceImage || "";
	...
	return (
		<div className={appointmentPageStyles.pageContainer}>
			<div className={appointmentPageStyles.maxWidthContainer}>
				<h1 className={appointmentPageStyles.doctorTitle}>My Doctor Appointments</h1>

				{error && <div className={appointmentPageStyles.loadingText}>{error}</div>}

				{loadingDoctors ? (
					<div className={appointmentPageStyles.loadingText}>Loading doctor appointments…</div>
				) : appointmentData.length === 0 ? (
					<div className={appointmentPageStyles.emptyStateText}>No doctor appointments</div>
				) : (
					<div className={appointmentPageStyles.doctorGrid}>
						{appointmentData.map((item) => (
							<div key={item.id} className={cardStyles.doctorCard}>
								<div className={cardStyles.doctorImageContainer} aria-hidden>
									<img
								...
								<div className="text-center w-full">
									<div className={cardStyles.doctorName}>{item.doctor}</div>
									...
									{item.status === "Rescheduled" && item.rescheduledTo ? (
										<div className={cardStyles.rescheduledText}>
											Rescheduled to <span className={cardStyles.rescheduledSpan}>{item.rescheduledTo.date} : {item.rescheduledTo.time}</span>
										</div>
									) : null}
								</div>
							</div>
				...
				<h1 className={appointmentPageStyles.serviceTitle} style={{ marginTop: 24 }}>My Service Appointments</h1>

				{loadingServices ? (
					<div className={appointmentPageStyles.serviceLoadingText}>Loading service appointments…</div>
				) : serviceData.length === 0 ? (
					<div className={appointmentPageStyles.serviceEmptyStateText}>No service appointments</div>
				) : (
