...
export default function AddService({ serviceId: propServiceId }) {
  ...
  // Auto-remove standard image error outline when preview exists
  useEffect(() => {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      fd.append("instructions", JSON.stringify(instructions.filter((ins) => ins.trim())));
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-orange-50 pb-4">
            <div>
              ...
              <p className={addServiceStyles.header.subtitle}>
                {serviceId
                  ? "Update service information and schedules"
                  : "Create and publish a new hospital service"}
              </p>
            </div>
